...
fd_rxdone0 = createInterruptFile(RXDONE0_INT, RISING);
fd_rxdone1 = createInterruptFile(RXDONE1_INT, RISING);
fd_rxdone2 = createInterruptFile(RXDONE2_INT, RISING);
fd_rxdone3 = createInterruptFile(RXDONE3_INT, RISING);

maxfdplusone = maxFdPlusOne(fd_rxdone0, fd_rxdone1, fd_rxdone2, fd_rxdone3);
...
lseek(fd_rxdone0, 0, SEEK_SET);
//reads GPIO value (2 bytes).
if (read(fd_rxdone0, & buffer, 2) != 2) {
perror("read");
}

// rewinds the file (necessary for next reading).
lseek(fd_rxdone1, 0, SEEK_SET);
//reads GPIO value (2 bytes).
if (read(fd_rxdone1, & buffer, 2) != 2) {
perror("read");
}

// rewinds the file (necessary for next reading).
lseek(fd_rxdone2, 0, SEEK_SET);
//reads GPIO value (2 bytes).
if (read(fd_rxdone2, & buffer, 2) != 2) {
perror("read");
}

// rewinds the file (necessary for next reading).
lseek(fd_rxdone3, 0, SEEK_SET);
//reads GPIO value (2 bytes).
if (read(fd_rxdone3, & buffer, 2) != 2) {
perror("read");
}
...
// at least one message received, scanning receivers (one pass)

//scanReceiversOnePass();
