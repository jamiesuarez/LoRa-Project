...
/****************************************************************************/
void hardwareSetup(int sel0, int sel1, int sel2, int cspin, int resetpin){
...
digitalWrite(RX_CSPIN, LOW);
(void)spiTransfer(address);
(void)spiTransfer(data);
digitalWrite(RX_CSPIN, HIGH);
}// end writeRegister()
...
bitClear(address, 7);   // Bit 7 cleared to write in registers
  ...
  // mais la datasheet semble erronée car elle fait référence
...
for(rxindex=0; rxindex<RXCOUNT; rxindex++){
	...
	reg_val = readRegister(REG_IRQ_FLAGS); // reads interruption flags

#ifdef DEBUG
fprintf(stdout,"step %d: reg_irq_flags %d= %x\n",++bugindex, rxindex, reg_val);
#endif


	if(bitRead(reg_val, 6) == 0x01){ // here, we have a message
	...
	fprintf(fd_log,"%09d: Message received on ", lastMessageIndex[rxindex]);
	...
	get_date(fd_log);
    	...
    	// Step 5a: test header
	 	...
	 	fprintf(fd_log,"Valid header received\n");
		} else {
	 	fprintf(fd_log,"No valid header received\n");
		}

	// Step 5b: test CRC
   		 if (bitRead(reg_val, 5) == 0x01){
	 	fprintf(fd_log,"Payload CRC error\n");
		} else {
	 	fprintf(fd_log,"Payload CRC OK\n");
		}
	...
	writeRegister(REG_IRQ_FLAGS, 0xff);  //  IRQ reset for this receiver

#ifdef DEBUG
fprintf(stdout,"step %d: reset flags of RX%d reg_irq_flags= %x \n",++bugindex,rxindex, readRegister(REG_IRQ_FLAGS));
#endif

	fclose(fd_index);
	fclose(fd_log);
	} // end if(bitRead(reg_val, 6) == 0x01) : if no message do nothing

} // end for(rxindex=0; rxindex<RXCOUNT; i++)
...
}// end getReceiver2(int rxindex)

/****************************************************************************/


/*
void getReceiver3(int rxindex){
...
fprintf(stdout,"step %d: reset flags of RX%d reg_irq_flags= %x \n",++bugindex,rxindex, readRegister(REG_IRQ_FLAGS));
	...
	if(reg_val != 0x00){ // here, we have something strange
	...
	if (bitRead(reg_val, 4)){
	fprintf(fd_log, "ERROR: Valid header received but no message");
...
int maxFdPlusOne(int fd0, int fd1, int fd2, int fd3){
