...
// test command input
if (argc != 3){
	fprintf(stdout, "usage: %s <number of channels> <timeout in us>\n", argv[0]);
	exit(EXIT_FAILURE);
}// end if (argc != 1)
...
if (read(fd_validheader, & buffer, 2) != 2) {
perror("read");
}
...
selectChannel(i);
	#ifdef DEBUG
fprintf(stdout,"step %d: reg_irq_flags= %02x\n",++bugindex, readRegister(REG_IRQ_FLAGS));
#endif
...
if (select(fd_validheader+1, NULL, NULL, & fds, NULL) < 0) {
	perror("select");
	break;
}
...
#ifdef DEBUG
fprintf(stdout,"step %d validHeaderState= %d\n",++bugindex, digitalRead(VALIDHEADER_INT));
#endif
