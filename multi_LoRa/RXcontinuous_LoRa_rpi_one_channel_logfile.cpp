...
unsigned int lastMessageIndex=0;
...
void loop() {
  ...
  while (true)
  {
	...
	if ((fd_index = fopen("/tmp/channel10.index", "w+")) < 0) {
		perror("index file:");
		exit(EXIT_FAILURE);
	}
	...
	lastMessageIndex++;
      ...
      fprintf(fd_index,"%09d\n", lastMessageIndex);

      fprintf(fd_log,"%09d: Message received on ", lastMessageIndex);
...
*/
	reg_val_table[i] = readRegister(REG_FIFO);
...
switch(channel){
case 0xD84CCC:
fprintf(fd_log,"channel 10, central freq = 865.20MHz");
break;
case 0xD86000:
fprintf(fd_log,"channel 11, central freq = 865.50MHz");
break;
case 0xD87333:
fprintf(fd_log,"channel 12, central freq = 865.80MHz");
break;
case 0xD88666:
fprintf(fd_log,"channel 13, central freq = 866.10MHz");
break;
case 0xD89999:
fprintf(fd_log,"channel 14, central freq = 866.40MHz");
break;
case 0xD8ACCC:
fprintf(fd_log,"channel 15, central freq = 866.70MHz");
break;
case 0xD8C000:
fprintf(fd_log,"channel 16, central freq = 867.00MHz");
break;
case 0xD90000:
fprintf(fd_log,"channel 16, central freq = 868.00MHz");
break;
default:
fprintf(fd_log,"unknown channel");
break;
}
